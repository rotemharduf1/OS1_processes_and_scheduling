...
truncate3(char *s)
{
  ...
  wait_wrapper(&xstatus);
  unlink("truncfile");
  exit_wrapper(xstatus);
}
...
// does exit_wrapper() call iput(p->cwd) in a transaction?
void
exit_wrapperiputtest(char *s)
{
  int pid, xstatus;
...
exectest(char *s)
{
  ...
  if (wait_wrapper(&xstatus) != pid) {
    printf("%s: wait_wrapper failed!\n", s);
  }
  if(xstatus != 0)
    exit_wrapper(xstatus);
...
// try to find any races between exit_wrapper and wait_wrapper
void
exit_wrapperwait_wrapper(char *s)
{
  int i, pid;
  ...
  for(i = 0; i < 100; i++){
    ...
    if(pid){
      int xstate;
      if(wait_wrapper(&xstate) != pid){
        printf("%s: wait_wrapper wrong pid\n", s);
        exit_wrapper(1);
      }
      if(i != xstate) {
        printf("%s: wait_wrapper wrong exit_wrapper status\n", s);
        exit_wrapper(1);
      }
    } else {
      exit_wrapper(i);
...
// code that handles a parent exit_wrappering
...
// what if two children exit_wrapper() at the same time?
void
twochildren(char *s)
{
...
// locking order when giving away a child to init, so that exit_wrapper()
// deadlocks against init's wait_wrapper()? also used to trigger a "panic:
// release" due to exit_wrapper() releasing a different p->parent->lock than
// it acquired.
void
reparent2(char *s)
{
...
forktest(char *s)
{
  ...
  for(; n > 0; n--){
    if(wait_wrapper(0) < 0){
      printf("%s: wait_wrapper stopped early\n", s);
      exit_wrapper(1);
    }
  }

  if(wait_wrapper(0) != -1){
    printf("%s: wait_wrapper got too many\n", s);
    exit_wrapper(1);
  }
}
...
struct test {
  ...
  {exit_wrapperiputtest, "exit_wrapperiput"},
  {iputtest, "iput"},
  {opentest, "opentest"},
  {writetest, "writetest"},
  {writebig, "writebig"},
  ...
  {preempt, "preempt"},
  {exit_wrapperwait_wrapper, "exit_wrapperwait_wrapper"},
  {reparent, "reparent" },
...
// run each test in its own process. run returns 1 if child's exit_wrapper()
// indicates success.
int
